...
QUESTION3 = """
SELECT * FROM
    (SELECT totals.day, (errors.count / totals.sum) AS percent FROM       --4
...
WHERE percents.percent > 0.01 ORDER BY day DESC;                          --5
"""
# 1 Calculate the total number of requests made each day by
#   summing together both response types (404 and 200)
# 2 Select out the number of requests that were errors made
#   each day (type 404 only)
# 3 Join the total number of requests made and number of
#   requests that were errors using the day to match them up
# 4 Calculate the error rate by dividing each days error
#   count by the total request count
