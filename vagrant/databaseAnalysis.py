...
QUESTION1 = """
SELECT title, count FROM article_views ORDER BY count DESC LIMIT %(limit)s;
"""


# Question 2: authors ranked by article views
QUESTION2 = """
SELECT named_authors.name, SUM(named_authors.count) FROM
(
...
# Question 3 requires view: daily_status
"""
...
SELECT * FROM
    ...
    FROM
        (SELECT SUM(count), day FROM daily_status GROUP BY day)           --1
        AS totals
        INNER JOIN                                                        --3
        (SELECT count, day FROM daily_status WHERE status='404 NOT FOUND')--2
        AS errors
